...
when ODIN_OS == .Windows {
	when ODIN_DEBUG && #exists("../libs/bgfxDebug.lib") {
		foreign import lib {"../libs/bgfxDebug.lib", "../libs/bimg_decodeDebug.lib", "../libs/bimgRelease.lib", "../libs/bxDebug.lib", "system:User32.lib", "system:Gdi32.lib"} // bimgDebug requires the `MTd` runtime
	} else {
		foreign import lib {"../libs/bgfxRelease.lib", "../libs/bimg_decodeRelease.lib", "../libs/bimgRelease.lib", "../libs/bxRelease.lib", "system:User32.lib", "system:Gdi32.lib"}
	}
} else when ODIN_OS == .Linux {
	when ODIN_DEBUG && #exists("../libs/libbgfxDebug.a") {
		foreign import lib {"../libs/libbgfxDebug.a", "../libs/libbimg_decodeDebug.a", "../libs/libbimgDebug.a", "../libs/libbxDebug.a", "system:stdc++"}
	} else {
		foreign import lib {"../libs/libbgfxRelease.a", "../libs/libbimg_decodeRelease.a", "../libs/libbimgRelease.a", "../libs/libbxRelease.a", "system:stdc++"}
	}
} else when ODIN_OS == .Darwin {
	// NOTE 2024-06-06 This is a guess, if you test this on macOS I would love to hear what libs/frameworks are required
	when ODIN_DEBUG && #exists("../libs/libbgfxDebug.a") {
		foreign import lib {"../libs/libbgfxDebug.a", "../libs/libbimg_decodeDebug.a", "../libs/libbimgDebug.a", "../libs/libbxDebug.a", "system:Cocoa.framework", "system:IOKit.framework", "system:OpenGL.framework", "system:QuartzCore.framework"}
	} else {
		foreign import lib {"../libs/libbgfxRelease.a", "../libs/libbimg_decodeRelease.a", "../libs/libbimgRelease.a", "../libs/libbxRelease.a", "system:Cocoa.framework", "system:IOKit.framework", "system:OpenGL.framework", "system:QuartzCore.framework"}
	}
} else {
...
CallbackVTable :: struct {
	...
	fatal:                  #type proc "c" (
		this: ^CallbackInterface, // [ent] Interface
	...
	trace_vargs:            #type proc "c" (
		...
		_arg_list: ^c.va_list,
	), // [in ] Variable arguments list initialized with va_start

	// Profiler region begin
	// !!ATTENTION!! not thread safe and can be called from any thread
	profiler_begin:         #type proc "c" (
		this: ^CallbackInterface, // [ent] Interface
	...
	cache_read:             #type proc "c" (
		...
		was_read: bool
	), // [out] True if data is read

	// Write cached item
	cache_write:            #type proc "c" (
		this: ^CallbackInterface, // [ent] Interface
		id: u64, // [in ] Cache id
	...
	screen_shot:            #type proc "c" (
		...
		yflip: bool,
	), // [in ] If `true`, image origin is bottom left

	// Called when a video capture begins
	capture_begin:          #type proc "c" (
		this: ^CallbackInterface, // [ent] Interface
		width: u32, // [in ] Image width
		height: u32, // [in ] Image height
		pitch: u32, // [in ] Number of bytes to skip between the start of each horizontal line of the image
...
AllocatorVTable :: struct {
	...
	realloc: #type proc "c" (
		this: ^AllocatorInterface, // [ent] Interface
...
foreign lib {
	...
	init_ctor :: proc(init: ^Init) --- // [out] Pointer to structure to be initialized

	// Initialize the bgfx library
	init :: proc(init:  /* const */^Init = nil,) -> (success: bool) --- // [in ] Initialization parameters// [out] If initialization was successful
...
foreign lib {
	...
	reset :: proc(width: u32, // [in ] Back-buffer width
	...
	frame :: proc(capture := false) -> (frame_number: u32) --- // [in ] Capture frame with graphics debugger// [out] Current frame number
...
foreign lib {
	...
	// Set debug flags
	set_debug :: proc(debug: DebugFlags) --- // Debug flags

	// Clear internal debug text buffer
	dbg_text_clear :: proc(attr := ANSIColor(0), // [in ] Background color
		small := false,) --- // [in ] Default 8x16 or 8x8 font

	// Print into internal debug text character-buffer (VGA-compatible text mode)
	dbg_text_printf :: proc(x, y: u16, // [in ] 2D position from top-left
    ...
    */
	...
	dbg_text_image :: proc(x, y: u16, // [in ] 2D Position from top-left
...
foreign lib {
	...
	get_supported_renderers :: proc(max := u8(0), // [in ] Maximum elements in enum array
		renderers: ^RendererType = nil,) -> (num_renderers: u8) --- // [out] Array where supported renderers will be written//       !!NOTE!! Originally called `_enum`// [out] Number of supported renderers

	// Returns name of renderer
	get_renderer_name :: proc(type: RendererType) -> (name: cstring) --- // [in ] Renderer backend// [out] Name of renderer
...
Caps :: struct {
	...
	gpu:              [4]struct {
		// Enumerated GPUs (bgfx_caps_gpu_t)
		vendor_id: PCI_ID, // Vendor PCI id
		device_id: u16, // Device id
	},
	limits:           struct {
		// Renderer runtime limits (bgfx_caps_limits_t)
...
foreign lib {
	...
	render_frame :: proc(msecs := i32(-1)) -> (status: RenderFrame) --- // [in ] Timeout in milliseconds (`-1` to disable timeout)// [out] Current renderer status

	// Set platform data
	// !!ATTENTION!! Must be called before init()
	set_platform_data :: proc(data:  /* const */^PlatformData,) --- // [in ] Platform data
	...
	// !!ATTENTION!! Must only be called by the render thread
	override_internal_texture_ptr :: proc(handle: TextureHandle, // [in ] Texture handle
		ptr: uintptr,) -> (texture: uintptr) --- // [in ] Native API pointer to texture// [out] Native API pointer, or `nil` if it is not create yet

	// Override internal texture by creating a new texture
	//   It’s expected you understand some bgfx internals before you use this call
	//   Previously created internal texture will released
	// !!ATTENTION!! Must only be called by the render thread
	override_internal_texture :: proc(handle: TextureHandle, // [in ] Texture handle
		width: u16, // [in ] Width
		height: u16, // [in ] Height
		num_mips: u8, // [in ] Number of mip-maps
		format: TextureFormat, // [in ] Texture format
		flags := TextureSettings{},) -> (texture: uintptr) --- // [in ] Texture settings// [out] Native API pointer, or `nil` if it is not create yet
...
foreign lib {
	...
	vertex_pack :: proc(input:  /* const */[4]f32, // [in ] Value to be packed into vertex stream
	...
	vertex_unpack :: proc(output: [4]f32, // [out] Result of unpacking
	...
	vertex_convert :: proc(dst_layout:  /* const */^VertexLayout, // [in ] Destination vertex stream layout
	...
	// Weld vertices
	weld_vertices :: proc(output: rawptr, // [out] Welded vertices remapping table, the size of buffer must be the same as number of vertices
		layout:  /* const */^VertexLayout, // [in ] Vertex stream layout
		...
		epsilon := f32(0.001),) -> (num_vertices: u32) --- // [in ] Error tolerance for vertex position comparison// [out] Number of unique vertices after vertex welding

	// Convert index buffer for use with different primitive topologies
	topology_convert :: proc(conversion: TopologyConvert, // [in ] Conversion type
		...
		index32: bool,) -> (num_vertices: u32) --- // [in ] Set to `true` if input indices are 32-bit// [out] Number of output indices after conversion

	// Sort indices
	topology_sort_tri_list :: proc(sort: TopologySort, // [in ] Sort order
		dst: rawptr, // [in ] Destination index buffer
	...
	set_palette_color_rgba8 :: proc(index: PaletteIndex, // [in ] Index into palette
		rgba: Color,) --- // [in ] Packed 32-bit RGBA value

	// Set palette color value
	@(link_name = "bgfx_set_palette_color")
	set_palette_color_float :: proc(index: PaletteIndex, // [in ] Index into palette
		rgba:  /* const */[4]f32,) --- // [in ] RGBA floating point value

	// Request screen shot of window back buffer
	// !!ATTENTION!! CallbackInterface.screen_shot must be implemented
	request_screen_shot :: proc(handle: FrameBufferHandle, // [in ] Frame buffer handle
...
foreign lib {
	...
	set_view_name :: proc(id: ViewID, // [in ] View id
	...
	set_view_rect_simple :: proc(id: ViewID, // [in ] View id
	...
	// Set view rectangle
	//   Draw primitive outside view will be clipped
	set_view_rect_ratio :: proc(id: ViewID, // [in ] View id
		x, y: u16, // [in ] 2D position from the top-left corner of the window
	...
	set_view_scissor :: proc(id: ViewID, // [in ] View id
	...
	set_view_clear :: proc(id: ViewID, // [in ] View id
	...
	set_view_clear_mrt :: proc(id: ViewID, // [in ] View id
		flags: ClearFlags, // [in ] Clear flags
	...
	set_view_mode :: proc(id: ViewID, // [in ] View id
		mode := ViewMode.Default,) --- // [in ] View sort mode

	// Set view frame buffer
	//   Not persistent after reset() call
	set_view_frame_buffer :: proc(id: ViewID, // [in ] View id
	...
	set_view_transform :: proc(id: ViewID, // [in ] View id
		view:  /* const */^matrix[4, 4]f32, // [in ] View matrix
		proj:  /* const */^matrix[4, 4]f32,) --- // [in ] Projection matrix

	// Post submit view reordering
	set_view_order :: proc(id := ViewID(0), // [in ] First view id
	...
	reset_view :: proc(id: ViewID) --- // [in ] View id
...
foreign lib {
	...
	encoder_begin :: proc(for_thread := false) -> (encoder: Encoder) --- // [in ] Explicitly request an encoder for a worker thread// [out] Encoder

	// End submitting draw calls from thread
	encoder_end :: proc(encoder: Encoder) --- // [ent] Encoder
...
foreign lib {
	...
	encoder_touch :: proc(this: Encoder, // [ent] Encoder
		id: ViewID,) --- // [in ] View id

	// Discard all previously set state for draw or compute call
	encoder_discard :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_set_marker :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_set_state :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_set_stencil :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_set_scissor_simple :: proc(this: Encoder, // [ent] Encoder
		x, y: u16, // [in ] 2D position from top-left of the window
		width: u16, // [in ] Width of scissor region
		height: u16,) -> (cache_index: u16) --- // [in ] Height of scissor region// [out] Scissor cache index

	// Set scissor from cache for draw primitive
	encoder_set_scissor_cached :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_alloc_transform :: proc(this: Encoder, // [ent] Encoder
		transform: ^Transform, // [out] Pointer to Transform structure
		num: u16,) -> (cache_index: u32) --- // [in ] Number of matrices// [out] Index into matrix cache

	// Set model matrix for draw primitive
	@(link_name = "bgfx_encoder_set_transform")
	encoder_set_transform_simple :: proc(this: Encoder, // [ent] Encoder
		mtx:  /* const */[^]matrix[4, 4]f32, // [in ] Pointer to first matrix in array
		num := u16(1),) -> (cache_index: u32) --- // [in ] Number of matrices in array// [out] Index into cache in case the same model matrix has to be used for other draw primitive call

	// Set model matrix from matrix cache for draw primitive
	encoder_set_transform_cached :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_set_condition :: proc(this: Encoder, // [ent] Encoder
...
encoder_set_index_buffer :: proc {
	encoder_set_index_buffer_simple,
...
encoder_set_vertex_buffer :: proc {
	encoder_set_vertex_buffer_simple,
...
encoder_set_instance_data_buffer :: proc {
	encoder_set_instance_data_buffer_simple,
...
foreign lib {
	...
	encoder_set_index_buffer_simple :: proc(this: Encoder, // [ent] Encoder
	...
	// Set index buffer for draw primitive
	encoder_set_dynamic_index_buffer :: proc(this: Encoder, // [ent] Encoder
		handle: DynamicIndexBufferHandle, // [in ] Dynamic index buffer
		first_index := u32(0), // [in ] First index to render
		num_indices := max(u32),) --- // [in ] Number of indices to render

	// Set index buffer for draw primitive
	encoder_set_transient_index_buffer :: proc(this: Encoder, // [ent] Encoder
	...
	encoder_set_vertex_buffer_simple :: proc(this: Encoder, // [ent] Encoder
	...
	encoder_set_dynamic_vertex_buffer :: proc(this: Encoder, // [ent] Encoder
		stream: u8, // [in ] Vertex stream
	...
	encoder_set_transient_vertex_buffer :: proc(this: Encoder, // [ent] Encoder
		stream: u8, // [in ] Vertex stream
	...
	encoder_set_vertex_count :: proc(this: Encoder, // [ent] Encoder
		num_vertices: u32,) --- // [in ] Number of vertices

	// Set instance data buffer for draw primitive
	@(link_name = "bgfx_encoder_set_instance_data_buffer")
	encoder_set_instance_data_buffer_simple :: proc(this: Encoder, // [ent] Encoder
	...
	// Set instance data buffer for draw primitive
	encoder_set_instance_data_from_vertex_buffer :: proc(this: Encoder, // [ent] Encoder
		handle: VertexBufferHandle, // [in ] Vertex buffer
		start_vertex: u32, // [in ] First instance data
		num: u32,) --- // [in ] Number of data instances

	// Set instance data buffer for draw primitive
	encoder_set_instance_data_from_dynamic_vertex_buffer :: proc(this: Encoder, // [ent] Encoder
		handle: DynamicVertexBufferHandle, // [in ] Dynamic vertex buffer
		start_vertex: u32, // [in ] First instance data
		num: u32,) --- // [in ] Number of data instances
	...
	encoder_set_instance_count :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_set_texture :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_set_uniform :: proc(this: Encoder, // [ent] Encoder
...
foreign lib {
	...
	encoder_submit_simple :: proc(this: Encoder, // [ent] Encoder
		id: ViewID, // [in ] View id
	...
	encoder_submit_occlusion_query :: proc(this: Encoder, // [ent] Encoder
		id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Program
	...
	encoder_submit_indirect :: proc(this: Encoder, // [ent] Encoder
		id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Program
	...
	encoder_submit_indirect_count :: proc(this: Encoder, // [ent] Encoder
		id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Program
		indirect_handle: IndirectBufferHandle, // [in ] Indirect buffer
...
encoder_set_compute_buffer :: proc {
...
foreign lib {
	...
	encoder_set_compute_index_buffer :: proc(this: Encoder, // [ent] Encoder
		stage: u8, // [in ] Compute stage
		handle: IndexBufferHandle, // [in ] Index buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute vertex buffer
	encoder_set_compute_vertex_buffer :: proc(this: Encoder, // [ent] Encoder
		stage: u8, // [in ] Compute stage
		handle: VertexBufferHandle, // [in ] Vertex buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute dynamic index buffer
	encoder_set_compute_dynamic_index_buffer :: proc(this: Encoder, // [ent] Encoder
		stage: u8, // [in ] Compute stage
		handle: DynamicIndexBufferHandle, // [in ] Dynamic index buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute dynamic vertex buffer
	encoder_set_compute_dynamic_vertex_buffer :: proc(this: Encoder, // [ent] Encoder
		stage: u8, // [in ] Compute stage
		handle: DynamicVertexBufferHandle, // [in ] Dynamic vertex buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute indirect buffer
	encoder_set_compute_indirect_buffer :: proc(this: Encoder, // [ent] Encoder
		stage: u8, // [in ] Compute stage
...
foreign lib {
	...
	encoder_set_image :: proc(this: Encoder, // [ent] Encoder
		stage: u8, // [in ] Texture unit
		handle: TextureHandle, // [in ] Texture handle
		mip: u8, // [in ] Mip level
...
foreign lib {
	...
	encoder_dispatch_simple :: proc(this: Encoder, // [ent] Encoder
		id: ViewID, // [in ] View id
	...
	encoder_dispatch_indirect :: proc(this: Encoder, // [ent] Encoder
		id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Compute program
		indirect_handle: IndirectBufferHandle, // [in ] Indirect buffer
		start := u32(0), // [in ] First element in indirect buffer
...
foreign lib {
	...
	encoder_blit :: proc(this: Encoder, // [ent] Encoder
		id: ViewID, // [in ] View id
...
foreign lib {
	...
	//   These empty draw calls will sort before ordinary draw calls
	//   Useful in cases when no other draw/compute primitive is submitted to view, but it’s desired to execute clear view
	touch :: proc(id: ViewID) --- // [in ] View id

	// Discard all previously set state for draw or compute call
	discard :: proc(flags := ~DiscardFlags{}) --- // [in ] Draw/compute states to discard
...
foreign lib {
	...
	// Sets a debug marker
	//   This allows you to group graphics calls together for easy browsing in graphics debugging tools
	set_marker :: proc(name: cstring, // [in ] Marker name
		len := max(i32),) --- // [in ] Marker name length
	//       If length is max(i32) name is treated as zero-terminated

	// Set shader debug name
	set_shader_name :: proc(handle: ShaderHandle, // [in ] Shader handle
	...
	set_texture_name :: proc(handle: TextureHandle, // [in ] Texture handle
...
_StateSettings_DepthTest :: enum uint {
	Disabled,
	Less,
	LEqual,
	Equal,
...
_StateSettings_PrimativeType :: enum uint {
	Tri_List,
	Tri_Strip,
	Line_List,
	Line_Strip,
...
STATE_DEFAULT :: StateSettings(
	0b00000_0_0_1_0000_0_000_00000000_0_1_0_1_0_0_000_000_0000_0000_0000_0000_0000_0001_1_1_1_1,
)
...
foreign lib {
	...
	// Set render states for draw primitive
	set_state :: proc(state: StateSettings, // [in ] State settings
		rgba := BlendFactor{},) --- // [in ] Sets blend factor used for StateSettings.blend_func_* of `.Factor` and `.Inv_Factor`
	//       When StateSettings.blend_independent is set, this is blend settings for render target's 1, 2, and 3
...
foreign lib {
	...
	// Set stencil test state
	set_stencil :: proc(fstencil: Stencil, // [in ] Front stencil state
		bstencil := STENCIL_NONE,) --- // [in ] Back stencil state
	//       If back is set to STENCIL_NONE fstencil is applied to both front and back facing primitives
...
foreign lib {
	...
	set_scissor_simple :: proc(x, y: u16, // [in ] 2D position from top-left of the window
		width: u16, // [in ] Width of scissor region
		height: u16,) -> (cache_index: u16) --- // [in ] Height of scissor region// [out] Scissor cache index

	// Set scissor from cache for draw primitive
	set_scissor_cached :: proc(cache := max(u16)) --- // [in ] Index in scissor cache
	//       If max(u16) use the view scissor instead
...
foreign lib {
	...
	// Reserve `num` matrices in internal matrix cache
	alloc_transform :: proc(transform: ^Transform, // [out] Pointer to Transform structure
		num: u16,) -> (cache_index: u32) --- // [in ] Number of matrices// [out] Index into matrix cache

	// Set model matrix for draw primitive
	@(link_name = "bgfx_set_transform")
	set_transform_simple :: proc(mtx:  /* const */[^]matrix[4, 4]f32, // [in ] Pointer to first matrix in array
		num := u16(1),) -> (cache_index: u32) --- // [in ] Number of matrices in array// [out] Index into cache in case the same model matrix has to be used for other draw primitive call

	// Set model matrix from matrix cache for draw primitive
	set_transform_cached :: proc(cache: u32, // [in ] Index in matrix cache
		num := u16(1),) --- // [in ] Number of matrices from cache
...
foreign lib {
	...
	// Set condition for rendering
	set_condition :: proc(handle: OcclusionQueryHandle, // [in ] Occlusion query handle
		visible: bool,) --- // [in ] Render if occlusion query is visible
...
set_instance_data_buffer :: proc {
	set_instance_data_buffer_simple,
...
foreign lib {
	...
	set_index_buffer_simple :: proc(handle: IndexBufferHandle, // [in ] Index buffer
		first_index := u32(0), // [in ] First index to render
		num_indices := max(u32),) --- // [in ] Number of indices to render

	// Set index buffer for draw primitive
	set_dynamic_index_buffer :: proc(handle: DynamicIndexBufferHandle, // [in ] Dynamic index buffer
		first_index := u32(0), // [in ] First index to render
		num_indices := max(u32),) --- // [in ] Number of indices to render

	// Set index buffer for draw primitive
	set_transient_index_buffer :: proc(tib:  /* const */^TransientIndexBuffer, // [in ] Transient index buffer
		first_index := u32(0), // [in ] First index to render
		num_indices := max(u32),) --- // [in ] Number of indices to render

	// Set vertex buffer for draw primitive
	@(link_name = "bgfx_set_vertex_buffer_with_layout")
	set_vertex_buffer_simple :: proc(stream: u8, // [in ] Vertex stream
		handle: VertexBufferHandle, // [in ] Vertex buffer
		start_vertex := u32(0), // [in ] First vertex to render
		num_vertices := max(u32), // [in ] Number of vertices to render
		layout_handle := VertexLayoutHandle.Invalid,) --- // [in ] Vertex layout for aliasing vertex buffer
	...
	set_dynamic_vertex_buffer :: proc(stream: u8, // [in ] Vertex stream
		handle: DynamicVertexBufferHandle, // [in ] Dynamic vertex buffer
		start_vertex := u32(0), // [in ] First vertex to render
		num_vertices := max(u32), // [in ] Number of vertices to render
		layout_handle := VertexLayoutHandle.Invalid,) --- // [in ] Vertex layout for aliasing vertex buffer
	...
	set_transient_vertex_buffer :: proc(stream: u8, // [in ] Vertex stream
		tvb:  /* const */^TransientVertexBuffer, // [in ] Transient vertex buffer
		start_vertex := u32(0), // [in ] First vertex to render
		num_vertices := max(u32), // [in ] Number of vertices to render
		layout_handle := VertexLayoutHandle.Invalid,) --- // [in ] Vertex layout for aliasing vertex buffer
	...
	// Set number of vertices for auto generated vertices use in conjunction with gl_VertexID
	//   !!ATTENTION!! Check Caps.supported for `.Vertex_ID`
	set_vertex_count :: proc(num_vertices: u32) --- // [in ] Number of vertices

	// Set instance data buffer for draw primitive
	@(link_name = "bgfx_set_instance_data_buffer")
	set_instance_data_buffer_simple :: proc(idb:  /* const */^InstanceDataBuffer, // [in ] Transient instance data buffer
		start: u32, // [in ] First instance data
		num: u32,) --- // [in ] Number of data instances

	// Set instance data buffer for draw primitive
	set_instance_data_from_vertex_buffer :: proc(handle: VertexBufferHandle, // [in ] Vertex buffer
		start_vertex: u32, // [in ] First instance data
		num: u32,) --- // [in ] Number of data instances

	// Set instance data buffer for draw primitive
	set_instance_data_from_dynamic_vertex_buffer :: proc(handle: DynamicVertexBufferHandle, // [in ] Dynamic vertex buffer
		start_vertex: u32, // [in ] First instance data
		num: u32,) --- // [in ] Number of data instances

	// Set number of instances for auto generated instances use in conjunction with gl_InstanceID
	//   !!ATTENTION!! Check Caps.supported for `.Vertex_ID` and `.Instancing`
	set_instance_count :: proc(num_instances: u32) --- // [in ] Number of instances
...
foreign lib {
	...
	// Set texture stage for draw primitive
	set_texture :: proc(stage: u8, // [in ] Texture unit
		sampler: UniformHandle, // [in ] Program sampler
		handle: TextureHandle, // [in ] Texture handle
		flags := SamplerSettings(0xFFFFFFFF),) --- // [in ] Texture sampling mode, default max(u32) value uses settings from texture
...
foreign lib {
	...
	// Set shader uniform parameter for draw primitive
	set_uniform :: proc(handle: UniformHandle, // [in ] Uniform
		value:  /* const */rawptr, // [in ] Pointer to uniform data
		num := u16(1),) --- // [in ] Number of elements
	//       Passing max(u16) will use the num passed on uniform creation
...
foreign lib {
	...
	submit_simple :: proc(id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Program
		depth := u32(0), // [in ] Depth for sorting
		flags := ~DiscardFlags{},) --- // [in ] Discard or preserve states

	// Submit primitive with occlusion query for rendering
	submit_occlusion_query :: proc(id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Program
		occlusion_query: OcclusionQueryHandle, // [in ] Occlusion query
		depth := u32(0), // [in ] Depth for sorting
		flags := ~DiscardFlags{},) --- // [in ] Discard or preserve states
	...
	// Submit primitive for rendering with index and instance data info from indirect buffer
	// !!ATTENTION!! Check Caps.supported for `.Draw_Indirect`
	submit_indirect :: proc(id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Program
		indirect_handle: IndirectBufferHandle, // [in ] Indirect buffer
		start := u32(0), // [in ] First element in indirect buffer
		num := u32(1), // [in ] Number of draws
	...
	// Submit primitive for rendering with index and instance data info and draw count from indirect buffers
	// !!ATTENTION!! Check Caps.supported for `.Draw_Indirect_Count`
	submit_indirect_count :: proc(id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Program
		indirect_handle: IndirectBufferHandle, // [in ] Indirect buffer
		start: u32, // [in ] First element in indirect buffer
		num_handle: IndexBufferHandle, // [in ] Buffer for number of draws
...
foreign lib {
	...
	// Set compute index buffer
	set_compute_index_buffer :: proc(stage: u8, // [in ] Compute stage
		handle: IndexBufferHandle, // [in ] Index buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute vertex buffer
	set_compute_vertex_buffer :: proc(stage: u8, // [in ] Compute stage
		handle: VertexBufferHandle, // [in ] Vertex buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute dynamic index buffer
	set_compute_dynamic_index_buffer :: proc(stage: u8, // [in ] Compute stage
		handle: DynamicIndexBufferHandle, // [in ] Dynamic index buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute dynamic vertex buffer
	set_compute_dynamic_vertex_buffer :: proc(stage: u8, // [in ] Compute stage
		handle: DynamicVertexBufferHandle, // [in ] Dynamic vertex buffer handle
		access: Access,) --- // [in ] Buffer access

	// Set compute indirect buffer
	set_compute_indirect_buffer :: proc(stage: u8, // [in ] Compute stage
		handle: IndirectBufferHandle, // [in ] Indirect buffer handle
		access: Access,) --- // [in ] Buffer access
...
foreign lib {
	...
	// Set compute image from texture
	set_image :: proc(stage: u8, // [in ] Texture unit
		handle: TextureHandle, // [in ] Texture handle
		mip: u8, // [in ] Mip level
		access: Access, // [in ] Texture access
		format := max(TextureFormat) + TextureFormat(1),) --- // [in ] Texture format
...
foreign lib {
	...
	dispatch_simple :: proc(id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Compute program
		num_x := u32(1), // [in ] Number of groups X
		num_y := u32(1), // [in ] Number of groups Y
		num_z := u32(1), // [in ] Number of groups Z
	...
	// Dispatch compute indirect
	dispatch_indirect :: proc(id: ViewID, // [in ] View id
		program: ProgramHandle, // [in ] Compute program
		indirect_handle: IndirectBufferHandle, // [in ] Indirect buffer
		start := u32(0), // [in ] First element in indirect buffer
		num := u32(1), // [in ] Number of dispatches
...
foreign lib {
	...
	// Blit texture 2D region between two 2D textures
	// !!ATTENTION!! Destination texture must be created with .blit_dst set to `true`
	blit :: proc(id: ViewID, // [in ] View id
		dst: TextureHandle, // [in ] Destination texture handle
		dst_mip: u8, // [in ] Destination texture mip level
		dst_x: u16, // [in ] Destination texture X position
		dst_y: u16, // [in ] Destination texture Y position
...
foreign lib {
	...
	alloc :: proc(size: u32,) -> ( /* const */memory: ^Memory) --- // [in ] Size to allocate// [out] New allocated memory

	// Allocate buffer and copy data into it
	//   Data will be freed inside bgfx
	copy :: proc(data:  /* const */rawptr, // [in ] Pointer to data to be copied
		size: u32,) -> ( /* const */memory: ^Memory) --- // [in ] Size of data to be copied// [out] New allocated memory
	...
	make_ref_static :: proc(data:  /* const */rawptr, // [in ] Pointer to data to be copied
		size: u32,) -> ( /* const */memory: ^Memory) --- // [in ] Size of data// [out] New allocated memory
	...
	make_ref_release :: proc(data:  /* const */rawptr, // [in ] Pointer to data to be copied
		...
		user_data: rawptr = nil,) -> ( /* const */memory: ^Memory) --- // [in ] User data to be passed to callback function// [out] New allocated memory
...
foreign lib {
	...
	create_shader :: proc(mem:  /* const */^Memory,) -> (shader: ShaderHandle) --- // [in ] Shader memory// [out] Shader handle

	// Returns the number of uniforms and uniform handles used inside a shader
	// !!ATTENTION!! Only non-predefined uniforms are returned
	get_shader_uniforms :: proc(handle: ShaderHandle, // [in ] Shader handle
		uniforms: [^]UniformHandle = nil, // [out] UniformHandle array where data will be stored
		max := u16(0),) -> (num_uniforms: u16) --- // [in ] Maximum capacity of array// [out] Number of uniforms used by shader

	// Destroy shader
	//   Once a shader program is created with `handle`, it is safe to destroy that shader
	destroy_shader :: proc(handle: ShaderHandle) --- // [in ] Shader handle

	// Create program with vertex and fragment shaders
	@(link_name = "bgfx_create_program")
	create_program_vert_frag :: proc(vsh: ShaderHandle, // [in ] Vertex shader
		fsh: ShaderHandle, // [in ] Fragment shader
		destroy_shaders := false,) -> (program: ProgramHandle) --- // [in ] If true, shaders will be destroyed when program is destroyed// [out] Program handle if vertex shader output and fragment shader input are matching
	//       Otherwise returns `.Invalid`

	// Create program with compute shader
	create_compute_program :: proc(csh: ShaderHandle, // [in ] Compute shader
		destroy_shaders := false,) -> (program: ProgramHandle) --- // [in ] If true, shader will be destroyed when program is destroyed// [out] Program handle

	// Destroy program
	destroy_program :: proc(handle: ProgramHandle) --- // [in ] Program handle
...
foreign lib {
	...
	create_uniform :: proc(name: cstring, // [in ] Uniform name in shader
		type: UniformType, // [in ] Type of uniform
		num := u16(1),) -> (uniform: UniformHandle) --- // [in ] Number of elements in array// [out] Handle to uniform object

	// Retrieve uniform info
	get_uniform_info :: proc(handle: UniformHandle, // [in ] Handle to uniform object
		info: ^UniformInfo,) --- // [out] Uniform info

	// Destroy shader uniform parameter
	destroy_uniform :: proc(handle: UniformHandle) --- // [in ] Handle to uniform object
...
create_dynamic_vertex_buffer :: proc {
	create_dynamic_vertex_buffer_empty,
...
foreign lib {
	...
	create_vertex_layout :: proc(layout:  /* const */^VertexLayout,) -> (layout_handle: VertexLayoutHandle) --- // [in ] Vertex layout// [out] Handle to layout

	// Destroy vertex layout
	destroy_vertex_layout :: proc(layout_handle: VertexLayoutHandle) --- // [in ] Vertex layout

	// Create static vertex buffer
	create_vertex_buffer :: proc(mem:  /* const */^Memory, // [in ] Vertex buffer data
		layout:  /* const */^VertexLayout, // [in ] Vertex layout
		flags := BufferSettings{},) -> (buffer_handle: VertexBufferHandle) --- // [in ] Buffer creation settings// [out] Static vertex buffer handle

	// Set static vertex buffer debug name
	set_vertex_buffer_name :: proc(handle: VertexBufferHandle, // [in ] Static vertex buffer handle
		name: cstring, // [in ] Static vertex buffer name
		len := max(i32),) --- // [in ] Static vertex buffer name length
	//       If length is max(i32) name is treated as zero-terminated

	// Destroy static vertex buffer
	destroy_vertex_buffer :: proc(handle: VertexBufferHandle) --- // [in ] Static vertex buffer handle

	// Start VertexLayout
	vertex_layout_begin :: proc(this: ^VertexLayout, // [ent] The layout being started
		renderer_type := RendererType.Noop,) -> (self: ^VertexLayout) --- // [in ] Renderer backend type// [out] Returns itself

	// Finalized VertexLayout
	vertex_layout_end :: proc(this: ^VertexLayout) --- // [ent] The layout being modified

	// Add attribute to VertexLayout
	//   Must be called between vertex_layout_begin() and vertex_layout_end()
	vertex_layout_add :: proc(this: ^VertexLayout, // [ent] The layout being modified
		...
		as_int := false,) -> (self: ^VertexLayout) --- // [in ] Packaging rule for vertex_pack, vertex_unpack, and vertex_convert for Uint8 and Int16//       Unpacking code must be implemented inside vertex shader// [out] Returns itself

	// Skip num bytes in vertex stream
	vertex_layout_skip :: proc(this: ^VertexLayout, // [ent] The layout being modified
		num: u8,) -> (self: ^VertexLayout) --- // [in ] Number of bytes to skip// [out] Returns itself

	// Retrieve an added attribute
	vertex_layout_decode :: proc(this:  /* const */^VertexLayout, // [ent] The layout being read
		attrib: Attrib, // [in ] Attribute semantics
	...
	vertex_layout_has :: proc(this:  /* const */^VertexLayout, // [ent] The layout being read
		attrib: Attrib,) -> (contains_attribute: bool) --- // [in ] Attribute semantics// [out] If VertexLayout contains attribute

	// Create empty dynamic vertex buffer
	@(link_name = "bgfx_create_dynamic_vertex_buffer")
	create_dynamic_vertex_buffer_empty :: proc(num: u32, // [in ] Number of vertices
		layout:  /* const */^VertexLayout, // [in ] Vertex layout
		flags := BufferSettings{},) -> (buffer_handle: DynamicVertexBufferHandle) --- // [in ] Buffer creation settings// [out] Dynamic vertex buffer handle

	// Create dynamic vertex buffer and initialize it
	create_dynamic_vertex_buffer_mem :: proc(mem:  /* const */^Memory, // [in ] Vertex buffer data
		layout:  /* const */^VertexLayout, // [in ] Vertex layout
		flags := BufferSettings{},) -> (buffer_handle: DynamicVertexBufferHandle) --- // [in ] Buffer creation settings// [out] Dynamic vertex buffer handle

	// Update dynamic vertex buffer
	update_dynamic_vertex_buffer :: proc(handle: DynamicVertexBufferHandle, // [in ] Dynamic vertex buffer handle
		start_vertex: u32, // [in ] Start vertex
		mem:  /* const */^Memory,) --- // [in ] Vertex buffer data

	// Destroy dynamic vertex buffer
	destroy_dynamic_vertex_buffer :: proc(handle: DynamicVertexBufferHandle) --- // [in ] Dynamic vertex buffer handle

	// Returns number of available vertices
	get_avail_transient_vertex_buffer :: proc(num: u32, // [in ] Number of required vertices
		layout:  /* const */^VertexLayout,) -> (num_vertices: u32) --- // [in ] Vertex layout// [out] Number of requested vertices or maximum available

	// Allocate transient vertex buffer
	alloc_transient_vertex_buffer :: proc(tvb: ^TransientVertexBuffer, // [out] Will be filled, valid for the duration of frame, and can be reused for multiple draw calls
...
create_dynamic_index_buffer :: proc {
	create_dynamic_index_buffer_empty,
...
foreign lib {
	...
	create_index_buffer :: proc(mem:  /* const */^Memory, // [in ] Index buffer data
		flags := BufferSettings{},) -> (buffer_handle: IndexBufferHandle) --- // [in ] Buffer creation settings// [out] Static index buffer handle

	// Set static index buffer debug name
	set_index_buffer_name :: proc(handle: IndexBufferHandle, // [in ] Static index buffer handle
		name: cstring, // [in ] Static index buffer name
		len := max(i32),) --- // [in ] Static index buffer name length
	//       If length is max(i32) name is treated as zero-terminated

	// Destroy static index buffer
	destroy_index_buffer :: proc(handle: IndexBufferHandle) --- // [in ] Static index buffer handle

	// Create empty dynamic index buffer
	@(link_name = "bgfx_create_dynamic_index_buffer")
	create_dynamic_index_buffer_empty :: proc(num: u32, // [in ] Number of indices
		flags := BufferSettings{},) -> (buffer_handle: DynamicIndexBufferHandle) --- // [in ] Buffer creation settings// [out] Dynamic index buffer handle

	// Create a dynamic index buffer and initialize it
	create_dynamic_index_buffer_mem :: proc(mem:  /* const */^Memory, // [in ] Index buffer data
		flags := BufferSettings{},) -> (buffer_handle: DynamicIndexBufferHandle) --- // [in ] Buffer creation settings// [out] Dynamic index buffer handle

	// Update dynamic index buffer
	update_dynamic_index_buffer :: proc(handle: DynamicIndexBufferHandle, // [in ] Dynamic index buffer handle
		start_index: u32, // [in ] Start index
		mem:  /* const */^Memory,) --- // [in ] Index buffer data

	// Destroy dynamic index buffer
	destroy_dynamic_index_buffer :: proc(handle: DynamicIndexBufferHandle) --- // [in ] Dynamic index buffer handle

	// Returns number of available indices
	get_avail_transient_index_buffer :: proc(num: u32, // [in ] Number of required indices
		index32 := false,) -> (num_indices: u32) --- // [in ] Set to `true` if input indices will be 32-bit// [out] Number of requested indices or maximum available

	// Allocate transient index buffer
	alloc_transient_index_buffer :: proc(tib: ^TransientIndexBuffer, // [out] Will be filled, valid for the duration of frame, and can be reused for multiple draw calls
	...
	alloc_transient_buffers :: proc(tvb: ^TransientVertexBuffer, // [out] Will be filled, valid for the duration of frame, and can be reused for multiple draw calls
		layout:  /* const */^VertexLayout, // [in ] Vertex layout
		...
		index32 := false,) -> (success: bool) --- // [in ] Set to `true` if input indices will be 32-bit// [out] Returns `true` if both buffers were allocated
...
_TextureSettings_RenderTarget :: enum uint {
	Disabled,
	No_MSAA,
	MSAA_x2,
	MSAA_x4,
...
_SamplerSettings_WrapMode :: enum uint {
	Repeat,
	Mirror,
	Clamp,
	Border,
}
_SamplerSettings_SamplingMode :: enum uint {
	Linear,
	Point,
}
_SamplerSettings_Compare :: enum uint {
	Disabled,
	Less,
	LEqual,
	Equal,
...
TEXTURE_SETTINGS_UV_CLAMP :: TextureSettings {
	sampler = SamplerSettings(0b0000_000_0_0000_0000_0_0_0_0_0_00_10_10),
}
...
foreign lib {
	...
	is_texture_valid :: proc(depth: u16, // [in ] Depth dimension of volume texture
		cube_map: bool, // [in ] Indicates that texture contains cubemap
		num_layers: u16, // [in ] Number of layers in texture array
		format: TextureFormat, // [in ] Texture format
		flags: TextureSettings,) -> (valid: bool) --- // [in ] Texture settings (no SamplerSettings)// [out] Returns `true` if a texture with the same parameters can be created

	// Calculate amount of memory required for texture
	calc_texture_size :: proc(info: ^TextureInfo, // [out] Resulting texture info structure
		width: u16, // [in ] Width
		height: u16, // [in ] Height
	...
	create_texture :: proc(mem:  /* const */^Memory, // [in ] DDS, KTX or PVR texture data
		flags := TextureSettings{}, // [in ] Texture creation and sampler settings
		skip := u8(0), // [in ] Skip top level mips when parsing texture
		info: ^TextureInfo = nil,) -> (texture_handle: TextureHandle) --- // [out] When not `nil` it returns parsed texture information// [out] Texture handle

	// Create 2D texture
	create_texture_2d :: proc(width: u16, // [in ] Width
		height: u16, // [in ] Height
		has_mips: bool, // [in ] Indicates that texture contains full mip-map chain
		num_layers: u16, // [in ] Number of layers in texture array
		...
		format: TextureFormat, // [in ] Texture format
		flags := TextureSettings{}, // [in ] Texture creation and sampler settings
		mem:  /* const */^Memory = nil,) -> (texture_handle: TextureHandle) --- // [in ] Texture data//       If not `nil`, created texture will be immutable//       If `nil` content of the texture is uninitialized//       When num_layers is more than 1, expected memory layout is texture and all mips together for each array element// [out] Texture handle

	// Create texture with size based on back-buffer ratio
	//   Texture will maintain ratio if back buffer resolution changes
	create_texture_2d_scaled :: proc(ratio: BackbufferRatio, // [in ] Frame buffer size in respect to back-buffer size
		has_mips: bool, // [in ] Indicates that texture contains full mip-map chain
		num_layers: u16, // [in ] Number of layers in texture array
		//       Must be 1 if Caps.supported does not have `.Texture_2D_Array`
		format: TextureFormat, // [in ] Texture format
		flags := TextureSettings{},) -> (texture_handle: TextureHandle) --- // [in ] Texture creation and sampler settings// [out] Texture handle

	// Update 2D texture
	update_texture_2d :: proc(handle: TextureHandle, // [in ] Texture handle
	...
	// Create 3D texture
	create_texture_3d :: proc(width: u16, // [in ] Width
		height: u16, // [in ] Height
		depth: u16, // [in ] Depth
		has_mips: bool, // [in ] Indicates that texture contains full mip-map chain
		format: TextureFormat, // [in ] Texture format
		flags := TextureSettings{}, // [in ] Texture creation and sampler settings
		mem:  /* const */^Memory = nil,) -> (texture_handle: TextureHandle) --- // [in ] Texture data//       If not `nil`, created texture will be immutable//       If `nil` content of the texture is uninitialized// [out] Texture handle

	// Update 3D texture
	update_texture_3d :: proc(handle: TextureHandle, // [in ] Texture handle
		mip: u8, // [in ] Mip level
		x: u16, // [in ] X offset in texture
		y: u16, // [in ] Y offset in texture
	...
	// Create Cube texture
	create_texture_cube :: proc(size: u16, // [in ] Cube side size
		has_mips: bool, // [in ] Indicates that texture contains full mip-map chain
		num_layers: u16, // [in ] Number of layers in texture array
		//       Must be 1 if Caps.supported does not have `.Texture_2D_Array`
		format: TextureFormat, // [in ] Texture format
	...
	// Update Cube texture
	update_texture_cube :: proc(handle: TextureHandle, // [in ] Texture handle
		layer: u16, // [in ] Layers in texture array
	...
	read_texture :: proc(handle: TextureHandle, // [in ] Texture handle
		data: rawptr, // [in ] Destination buffer
		mip := u8(0),) -> (frame_number: u32) --- // [in ] Mip level// [out] Frame number when the result will be available, see frame()
	...
	get_direct_access_ptr :: proc(handle: TextureHandle) -> (texture_data_ptr: rawptr) --- // [in ] Texture handle// [out] Pointer to texture memory
	...
	// Destroy texture
	destroy_texture :: proc(handle: TextureHandle) --- // [in ] Texture handle
...
foreign lib {
	...
	attachment_init :: proc(this: ^Attachment, // [ent] The attachment being initialized
	...
	is_frame_buffer_valid :: proc(num: u8, // [in ] Number of attachments
		attachment:  /* const */^Attachment,) -> (valid: bool) --- // [in ] Attachment texture info// [out] Returns `true` if a frame buffer with the same parameters can be created

	// Create frame buffer (simple)
	@(link_name = "bgfx_create_frame_buffer")
	create_frame_buffer_simple :: proc(width: u16, // [in ] Texture width
		height: u16, // [in ] Texture height
		format: TextureFormat, // [in ] Texture format
		texture_flags := TEXTURE_SETTINGS_UV_CLAMP,) -> (frame_buffer_handle: FrameBufferHandle) --- // [in ] Texture creation and sampler settings// [out] Handle to frame buffer object
	...
	create_frame_buffer_scaled :: proc(ratio: BackbufferRatio, // [in ] Frame buffer size in respect to back-buffer size
		format: TextureFormat, // [in ] Texture format
		texture_flags := TEXTURE_SETTINGS_UV_CLAMP,) -> (frame_buffer_handle: FrameBufferHandle) --- // [in ] Texture creation and sampler settings// [out] Handle to frame buffer object

	// Create MRT frame buffer from texture handles (simple)
	create_frame_buffer_from_handles :: proc(num: u8, // [in ] Number of texture attachments
		handles:  /* const */^TextureHandle, // [in ] Texture attachments
		destroy_texture := false,) -> (frame_buffer_handle: FrameBufferHandle) --- // [in ] If `true`, textures will be destroyed when frame buffer is destroyed// [out] Handle to frame buffer object

	// Create frame buffer for multiple window rendering
	// !!ATTENTION!! Frame buffer cannot be used for sampling
	create_frame_buffer_from_nwh :: proc(nwh: rawptr, // [in ] OS’ target native window handle
		...
		depth_format := max(TextureFormat) + TextureFormat(1),) -> (frame_buffer_handle: FrameBufferHandle) --- // [in ] Window back buffer depth format// [out] Handle to frame buffer object

	// Create MRT frame buffer from texture handles with specific layer and mip level
	create_frame_buffer_from_attachment :: proc(num: u8, // [in ] Number of texture attachments
		attachment:  /* const */[^]Attachment, // [in ] Attachment texture info
		destroy_texture := false,) -> (frame_buffer_handle: FrameBufferHandle) --- // [in ] If `true`, textures will be destroyed when frame buffer is destroyed// [out] Handle to frame buffer object

	// Obtain texture handle of frame buffer attachment
	get_texture :: proc(handle: FrameBufferHandle, // [in ] Frame buffer handle
		attachment := u8(0),) -> (texture_handle: TextureHandle) --- // [in ] Frame buffer attachment index// [out] Returns `.Invalid` if attachment index is not correct, or frame buffer is created with native window handle

	// Set frame buffer debug name
	set_frame_buffer_name :: proc(handle: FrameBufferHandle, // [in ] Frame buffer handle
		name: cstring, // [in ] Frame buffer name
		len := max(i32),) --- // [in ] Frame buffer name length
	//       If length is max(i32) name is treated as zero-terminated

	// Destroy frame buffer
	destroy_frame_buffer :: proc(handle: FrameBufferHandle) --- // [in ] Frame buffer handle
...
foreign lib {
	...
	get_avail_instance_data_buffer :: proc(num: u32, // [in ] Number of required instances
		stride: u16,) -> (num_slots: u32) --- // [in ] Stride per instance// [out] Number of requested instance buffer slots or maximum available

	// Allocate instance data buffer
	alloc_instance_data_buffer :: proc(idb: ^InstanceDataBuffer, // [out] Will be filled, valid for the duration of frame, and can be reused for multiple draw calls
...
foreign lib {
	...
	create_indirect_buffer :: proc(num: u32) -> (buffer_handle: IndirectBufferHandle) --- // [in ] Number of indirect calls// [out] Indirect buffer handle

	// Destroy draw indirect buffer
	destroy_indirect_buffer :: proc(handle: IndirectBufferHandle) --- // [in ] Indirect buffer handle
...
foreign lib {
	...
	get_result :: proc(handle: OcclusionQueryHandle, // [in ] Handle to occlusion query object
		result: ^i32 = nil,) -> (occlusion_result: OcclusionQueryResult) --- // [out] Number of pixels that passed test//       This argument can be `nil` if result of occlusion query is not needed// [out] Occlusion query result

	// Destroy occlusion query
	destroy_occlusion_query :: proc(handle: OcclusionQueryHandle) --- // [in ] Handle to occlusion query object
